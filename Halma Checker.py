...
endPlace2={(15,15),(15,14),(15,13),(15,12),(15,11),(14,15),(14,14),(14,13),(14,12),(14,11),(13,15),(13,14),(13,13),(13,12),(12,15),(12,14),(12,13),(11,15),(11,14)}
...
def getAction(state):
    ...
    player =state[0]
    if player == piece1:  # playing from bottom to top, hence later vertical value is smaller
...
while True:
    if flag==1:
        print('x')
        if isEnd(piece1,matrix):
            break
        else:
            ff.write('\n')
            ff.write(str(count1)+' ' + 'BLACK')
            count1+=1
            ff.write('\n')
            action = MinMaxValue((piece1, matrix), 2)
            matrix[action[0][0]][action[0][1]]='.'
            matrix[action[-1][0]][action[-1][1]]='B'
        ...
        if isEnd(piece2,matrix):
            break
        else:
            ff.write('\n')
            ff.write(str(count2)+' '+'WHITE')
            count2+=1
            ff.write('\n')
            action =getAction((piece2, matrix))
            matrix[action[0][0]][action[0][1]] = '.'
            matrix[action[-1][0]][action[-1][1]] = 'W'
            if abs(action[-1][0] - action[0][0]) >= 2 or abs(action[-1][1] - action[0][1]) >= 2:
                move = 'J'
            else:
                move = 'E'
